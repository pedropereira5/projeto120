...
# Exemplo de lista de perguntas e respostas (você deve substituir com seus próprios dados)
perguntas = ["Qual é o seu produto?", "Como faço para retornar um item?", "Vocês oferecem frete grátis?"]
respostas = ["Nós vendemos roupas e acessórios.", "Você pode retornar um item dentro de 30 dias após a compra.", "Sim, oferecemos frete grátis para compras acima de R$ 100,00."]

# Tokenização e vetorização das perguntas
vectorizer = CountVectorizer()
X = vectorizer.fit_transform(perguntas)

# Treinamento do modelo de regressão logística
model = LogisticRegression()
model.fit(X, respostas)

# Loop infinito para interagir com o chatbot
while True:
    # Obtenção da entrada do usuário
    user_input = input("Usuário: ")

    # Tokenização, stemização e codificação da entrada do usuário
    user_input_tokens = vectorizer.transform([user_input])

    # Previsão da resposta do modelo
    predicted_class = model.predict(user_input_tokens)

    # Exibição de uma resposta aleatória da classe prevista
    responses = [respostas[i] for i, classe in enumerate(respostas) if classe == predicted_class]
